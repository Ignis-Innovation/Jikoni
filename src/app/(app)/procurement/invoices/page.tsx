...
export default async function InvoicesList() {
  ...
  const { data: rows } = await supabase
    ...
    .select("id, code, amount_minor, currency_code, status, match_status, due_date, created_at, parties:vendor_party_id(display_name)")
    .is("deleted_at", null)
          ...
          <tbody className="divide-y divide-zinc-100">
            ...
            {((rows ?? []) as any[]).map((r) => (
              <tr key={r.id} className="hover:bg-zinc-50">
                <td className="px-4 py-2.5 font-medium text-emerald-700"><Link href={`/procurement/invoices/${r.id}`}>{r.code ?? "—"}</Link></td>
