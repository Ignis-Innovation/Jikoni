...
export default async function RequisitionsList() {
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-zinc-100">
            {(rows ?? []).map((r) => (
              <tr key={r.id} className="cursor-pointer hover:bg-zinc-50">
                <td className="px-4 py-2.5 font-medium text-emerald-700">
                  <Link href={`/procurement/requisitions/${r.id}`}>{r.code ?? "—"}</Link>
                </td>
