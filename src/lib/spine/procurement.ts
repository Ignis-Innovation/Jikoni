...
/** Receive goods against a PO: GRN + update received qty + recompute PO status. */
export async function receivePO(
  ...
  const { data: grn, error: grnErr } = await supabase
    .from("grns")
    .insert({ po_id: poId, received_by: me, status: "received" })
    .select("id")
    .single();
  ...
  for (const r of receipts.filter((x) => x.qty > 0)) {
    ...
    const { data: line } = await supabase.from("po_lines").select("qty_received").eq("id", r.po_line_id).single();
    await supabase.from("po_lines").update({ qty_received: Number(line?.qty_received ?? 0) + r.qty }).eq("id", r.po_line_id);
  }

  const { data: lines } = await supabase.from("po_lines").select("qty_ordered, qty_received").eq("po_id", poId);
  const all = lines ?? [];
