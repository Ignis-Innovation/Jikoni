...
export function PartiesView({ caps }: { caps: Caps }) {
  ...
  return (
    <div className="w-full">
              ...
              <TR key={p.id}>
                ...
                <TD className="text-right">
                  <div className="flex justify-end gap-1">
                    ...
                    {caps.del && (
                      <button onClick={() => onDelete(p)} className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive" aria-label="Delete">
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 rounded-lg bg-foreground px-4 py-2 text-sm text-background shadow-lg">{toast}</div>
      )}
    </div>
  );
}
